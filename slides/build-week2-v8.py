...
loop_slide("建立架構", "three-loops-mermaid-h2.png",
           [{"text": "把 AI 說的話順成自己說的話，把真實的肉長出來。", "bold": True, "after": 12},
            ...
            {"text": "文稿像文章，投影片分口述跟畫面，節奏不一樣。", "color": MUTED}],
           "一開始可以先叫 AI 打個草稿，這裡需要人大量參與，補文獻來源或圖片。越快進到具體改起來越慢。")

# ============ 12 實例：補進去的想法 ============
s = assert_slide("建立架構：文稿裡補進去的想法", "三個迴圈")
...
tb(s, 9.5, BODY_Y + 0.85, W - M - 9.8, 2.2, ["這些段落 agent 原本沒有，", "是討論時補進文稿的。"], size=17, bold=True, spacing=6)
notes(s, "我中間一直在來回修那個文稿。agent 沿論點展開的是通順的字，觀點是人插進去的。改語氣是另一回事，在第六步。")
...
step_slide(num_cn[3], "起草文稿",
           ...
           "文稿比較像文章，可以作為輔助使用。挑一句查證是試跑後加的動作，學員不會主動做，第 50 分鐘全場再示範一次。")

step_slide(num_cn[4], "分成逐頁稿",
           ["請 agent 找投影片該怎麼分頁的原則，挑出要用的。", "先只列每頁一句標題，連讀通過才往下。", "再逐頁配證據與口述，存成分頁文案。"],
           "「先只列每頁一句標題，我連讀過再配內容。」",
           ["只讀標題能講完故事。", "每頁對應草稿一段，對不上的刪；第一頁不能省。"], 2, 10, ev_ghost,
           "分頁方法找到好幾種，本課選綜合的一種。先讓 agent 自己找一輪再對照課程筆記。文稿更具體後做成分頁草稿，投影片分口述跟畫面。")
