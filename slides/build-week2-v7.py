#!/usr/bin/env python3
"""第二週講師版投影片，依 drafts/08-week2-slide-copy.md 第十版（28 頁）建置。

用法：python3 slides/build-week2-v7.py  → slides/week2-agent-knowledge-work-v7.pptx
...
A = ROOT / "assets"
IC = A / "week2-icons"
OUT = ROOT / "slides" / "week2-agent-knowledge-work-v7.pptx"
...
CROPS = {"three-loops-mermaid-h1.png": (0.0, 0.318), "three-loops-mermaid-h2.png": (0.318, 0.598), "three-loops-mermaid-h3.png": (0.636, 0.925)}
...
# ============ 02 段：怎麼學 agent ============
section_slide("怎麼學 agent", sub="前半段 30 分", note="五頁：AI 是放大器、把 AI 當同事、從自己需要補足的地方開始、交付出去的東西是要負責的、東西都要外部化存起來。")
...
loop_slide("內容要走三個迴圈", "three-loops-mermaid.png",
           [{"text": "確認方向、確認內容、確認表達，三圈各有人的判斷（紅框）。", "after": 12},
            {"text": "灰色虛線在圈內回頭；紅色虛線退回上一圈。", "after": 12},
            {"text": "跟軟體開發對得上：需求探索、建立架構、寫程式。", "color": MUTED}],
           "前半段是訂方向，後半段是確認內容，會看到兩個大的迴圈；內容確定了還有第三圈，讓交付物對齊自己的標準。接下來每圈兩頁。")
...
mini_loop(s, "three-loops-mermaid-h1.png")
...
# ============ 12 確認內容 ============
loop_slide("確認內容", "three-loops-mermaid-h2.png",
...
# ============ 13 實例：講稿 ============
s = assert_slide("確認內容：把講稿修成自己的觀點", "三個迴圈")
mini_loop(s, "three-loops-mermaid-h2.png")
rows = [["agent 沿論點展開的句子", "改成自己的觀點"],
        ["這門課不是教 agent，也不是教做簡報，是教每件工作本來怎麼做", "能看出問題的人，都是原本就清楚一份簡報該有什麼的人"],
        ["下週帶著走", "供下週沿用"],
        ["哪幾段得自己盯", "哪些環節必須自行把關"]]
table(s, M, BODY_Y + 0.15, W - 2 * M, rows, [5.8, W - 2 * M - 5.8], size=14, row_h=0.9, first_col_bold=False)
tb(s, M, BODY_Y + 4.05, W - 2 * M, 0.8, "改的紀錄留在文案語氣規則檔，下次 agent 就照著寫。", size=13, color=MUTED)
notes(s, "我中間一直在來回修那個講稿。agent 沿論點展開的字，一個一個改成自己的看法；試跑與收斂是這一圈的另一面。")

# ============ 14 確認表達 ============
loop_slide("確認表達", "three-loops-mermaid-h3.png",
...
# ============ 15 實例：ghost deck ============
s = assert_slide("確認表達：看 ghost deck 重新分頁", "三個迴圈")
mini_loop(s, "three-loops-mermaid-h3.png")
w = W - 2 * M
rect(s, M, BODY_Y + 0.15, w, 2.6)
tb(s, M + 0.3, BODY_Y + 0.3, w - 0.6, 0.35, "同一頁標題的三個版本", size=13, bold=True, color=ACCENT)
vers = [("生成的", "還沒有方法時請教，已知道要做什麼時交辦", MUTED, False), ("原話當底", "聊天像請教導師", DARK, False), ("再改", "把 AI 當同事", ACCENT, True)]
for i, (lab, t, col, b) in enumerate(vers):
    y = BODY_Y + 0.75 + i * 0.6
    tb(s, M + 0.3, y, 1.4, 0.5, lab, size=12, color=MUTED, anchor="m")
    tb(s, M + 1.7, y, w - 2.0, 0.5, t, size=17, color=col, bold=b, anchor="m")
tb(s, M, BODY_Y + 3.0, w, 0.35, "這幾天在 ghost deck 上做的事", size=13, bold=True, color=ACCENT)
tb(s, M, BODY_Y + 3.4, w, 1.6, [{"text": t, "bullet": True} for t in ["只看標題連讀，刪頁、換順序、縮標題、加段落頁", "再看逐頁稿，確定每頁的畫面與口述", "最後才排版；內容問題都在前兩步解決"]], size=14, spacing=6)
notes(s, "先把 ghost deck 寫好，這個大綱就是 ghost deck 的底；用原話來調整 ghost deck 比較能看懂。現在正在做的就是這一圈，畫面是最後一步。")
...
def ev_ghost(s, x, y, w, h):
    ...
    lines = ["2 研究專案裡的材料、想法與成果持續累積", "3 你卡在哪裡，由兩種能力決定", "4 缺的是哪種能力，就需要哪種協助", "5 還沒有方法時請教，已知道要做什麼時交辦", "6 先確認方向，再確認內容，最後排版", "7 過程保存成三類檔案"]
