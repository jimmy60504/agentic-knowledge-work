...
每圈同一個形狀：拿判準 → agent 產出 → 人檢查 → 通過，或改了再產，或退回前一圈。退回前一圈永遠比本圈再產貴，所以便宜的先做，內容留在 Markdown。
...
2. **要不要多一圈**：候選是前面的「累積」（沒有任務時 kb 也在長）與後面的「回存」（判準補進 AGENTS.md、棄案留 drafts）。
3. **語意要不要更精準**：判準只畫在第三圈，其實每圈入口都是判準；「連讀」「試講」是簡報用語，換成簽呈或報告時怎麼說。
