...
> **待取得研究圖：** 原論文 Figure 3「Heterogeneity of AI Impact, by Skill and Tenure」，[全文與圖說](https://arxiv.org/html/2304.11771v2)。用途是讓聽眾看出不同技能與年資的效果差異。下一步從 PDF 取得原圖，確認座標、分組、信賴區間及投影可讀性；不得自行補畫未核對的數值。v2 授權為 [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/)，使用時核對用途條件，保留原圖與作者資訊，不直接翻譯改圖後發布。目前只完成圖說定位，尚未擷取。

講師可在自繪摘要與研究原圖中擇一呈現，避免連放兩頁重述。若採用原圖，中文解釋另列在圖旁。
...
以下為「從自己需要補足的地方開始」的候選版面，屬文字排版草圖，尚未製作投影片。研究圖仍待取得，可以先用前半段的自繪摘要示意討論內容。

```text
┌────────────────────────────────────────────┐
│ AI 也可能協助補足經驗                       │
│                                            │
│  [研究 Figure 3，待取得]   較少經驗者也受益  │
│  保留原圖座標與圖例       先找自己的需要     │
│                                            │
...
- 本次主線選用放大鏡照片、客服研究的自繪摘要與 Figure 3 候選。槓桿 SVG 留在 [素材筆記](../kb/ai-amplifier-evidence-and-visuals.md) 作備選，避免引入物理公式分散注意；METR 更新留作引用需核對時效的備講例子。素材依教學用途取捨，不將整批蒐集結果全部放入課堂。
