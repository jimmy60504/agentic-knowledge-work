...
用熟悉主題或既有簡報，練習請 agent 補充想法、完成一段操作與協助檢查，再由自己選擇和修改。前半段說明內容演變與人的判斷，後半段用同一任務實際操作 `AGENTS.md`、`kb/`、`drafts/` 與簡報輸出，讓流程留下可讀、可修改的檔案。
...
| 後半段 | 講師操作一段、學員跟做一段：建立工作資料夾 → 保存素材 → 交代規則 → 故事線與逐頁草稿 → 人充實內容 → 排版、試講與修改 |
...
- 測試不用機密、個資與未公開公務資料。課堂建立簡單的 `AGENTS.md`、`kb/` 與 `drafts/`，保存必要中間成果；不要求 GitHub push 或課後作業。
...
本次先完成一則素材筆記、一份簡短規則、故事線與逐頁草稿，以及三到五頁可編輯簡報；至少深入修改與試講其中一頁。檔案與資料夾名稱為本次教案的建議，後續可調整。
...
講師先示範用 agent 開啟或指定一個獨立練習資料夾，再請它建立以下結構。學員跟著做，完成後打開資料夾確認。
...
> 這是本次簡報練習的資料夾。請建立 `kb/原始材料/`、`drafts/` 與 `output/`，保留我已有的檔案；先不要製作投影片。我的主題是＿＿，對象是＿＿，希望他們聽完能＿＿。

有舊簡報就把副本放進原始材料；只有想法就先寫下三到五個原有重點。先確認 agent 正在操作的資料夾，以及它是否能讀寫檔案；若環境尚未就緒，使用講師準備的起始資料夾排除問題。

#### 二、把查到的材料整理進 kb

先請 AI 提出可能的角度與關鍵字，再選一個方向查閱。講師示範讀取一份材料、留下來源，學員用同一方式整理一則；當場無法查找時，使用講師提供的公開材料。

> 請依我提供的材料，提出值得繼續了解的角度與關鍵字。選定方向後，再整理到 `kb/主題素材.md`，分成來源、重點、自己的洞見、課程／簡報用途（候選）四段。只整理實際讀到的內容，缺資料就標記待查。
...
> 請依剛才確認的內容，在這個練習資料夾建立簡短的 `AGENTS.md`：寫明簡報的受眾與目的、材料在 `kb/`、草稿在 `drafts/`、輸出在 `output/`，以及用語和檢查要求。規則包括保留既有事實、不編造來源、不把待查資料寫成定論、保留原稿並另存輸出。有不清楚的地方先列出來。
...
回到資料夾看一次：`kb/` 留下資料與洞見，`AGENTS.md` 留下要求，`drafts/` 留下故事線與逐頁內容，`output/` 留下可以打開檢查的成果。
