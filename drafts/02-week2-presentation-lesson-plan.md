...
| 想法如何逐漸變成一份簡報 | 以演變圖呈現探索到成品；用短迴圈先確認方向，在 Markdown 修改內容，最後排版 | 正式授課簡報 |
...
做這個流程時，每次先處理一個能看完、能判斷的部分。方向還不清楚，可以先請 agent 在對話裡講一遍準備怎麼做，自己看過、討論並修正，確認後再整理成 Markdown 存下來。

內容還在變動時，盡量留在 Markdown 修改。先改一段論點、一條故事線或一頁文字，確認意思與順序，再往下展開。這樣每次改動的範圍比較小，也容易看出改了什麼。

如果太早把整份簡報的圖文與版面做細，後來發現方向要改，連帶要重做的東西就多了。原本只要在文字裡換一段順序，可能變成重排好幾頁、換圖與重新調整文字。先用對話和 Markdown 把內容改清楚，能減少投入美編後才整批返工的情況。

可以依這個節奏前進：**對話確認做法 → Markdown 修改內容 → 確認後排版 → 試講微調**。確認是確認眼前這一小段可以繼續，不必等整份內容毫無疑問才保存；未定事項也可以標記存下來。排版後如果發現方向有問題，就回到故事線或文字草稿修正，再更新簡報。

講師帶法：在流程圖上指出三個較小的修改範圍——對話中的方向、Markdown 中的內容、簡報中的呈現。後半段先讓 agent 說明做法，再確認一段內容，最後才展開排版，讓學員看到每次只處理眼前的問題。
...
請 agent 讀取已保存的素材、自己的洞見與工作要求，先在對話裡講一遍準備怎麼安排重點與順序。先討論這條線，確認後再請它落檔。
...
交代這次的受眾、內容範圍與頁數，請 agent 讀取已保存的要求，沿故事線先起草一頁文字。看過寫法再展開其餘頁面，內容修改先留在 Markdown，暫不製作簡報版面。
...
先讀過 Markdown 草稿，確認本次要輸出的重點、順序與文字；待定的主張先補清楚或移出輸出範圍。再請 agent 製作可編輯簡報，說明頁數、範圍與必須遵守的格式，讓它提出版面做法。已有指定版型就提供給它。

例如：「用剛才的內容，做成五頁以內、可以接著編輯的 PowerPoint，給新同仁看。進階情況這次先不放，版面你先試一個適合的做法。」

可以先看一頁版面，確認合用再套到其餘頁面。請它另存輸出，保留原稿；本次示範可建立 `output/`。打開成果，試著選取文字、改一個字，再實際講一頁，指出看不清楚、講不順或意思不對的地方，請它修改後另存。

比較修改前後，確認內容與版面合用。若方向或故事線有問題，先回到 Markdown 修正，再請 agent 更新簡報；單純版面問題就修改該頁。內容有改就同步更新逐頁草稿；值得沿用的要求補進 `AGENTS.md`。工具無法輸出 PPTX 時，先做一頁預覽，搭配講師備援檔檢查。
