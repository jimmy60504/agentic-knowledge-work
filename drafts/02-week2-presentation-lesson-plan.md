...
以研究簡報為練習，讓學員看到討論如何接到搜尋、保存材料、整理 draft 與製作 PPT。人決定方向、採用哪些內容與如何修改，agent 接續執行，把想講的內容做成自己能說明、能接續編輯的成果。

| 段落 | 安排 |
|---|---|
| 開場 | 同一份研究簡報的三個狀態：初步想法 → 有依據的 draft → 可編輯 PPT |
...
做研究簡報時，可能已經知道大致想講什麼，但還缺支持論點的材料；內容整理好了，也還有圖表、分頁與排版要處理。今天用同一份簡報，先看它如何從初步想法走到可以使用的成果。

| 預覽畫面 | 要讓學員看到什麼 |
|---|---|
| 初步主張與少量材料 | 指出一個內容缺口，例如缺少來源、圖表或能說明主張的例子。 |
| 補入依據的 draft | Agent 協助查找與整理，人決定採用什麼、如何解讀，並保留來源；內容已能說明。 |
| 可編輯 PPT | Agent 依確認內容安排圖文，人檢查與修改；展示同一段內容如何成為投影片。 |

先讓學員指出第一個畫面缺什麼，再揭示後兩個狀態。重點放在內容如何充實，以及哪些製作工作可以交辦；不以樸素版面作為失敗示例。

口述接回本週核心：「今天我們會讓 agent 把想法做成成果。你決定想講什麼、哪些材料值得用，它接著搜尋、保存、整理與製作；看到成果，再一起修改。」

> **準備註記：** 三個狀態須來自同一個公開研究題材的實際預作，標明為課前準備；目前題材與預覽檔待備妥。開場只短暫展示變化，後半另開空專案完整操作。這組預覽是開場教材，與工具失效時才啟用的備援成果分開。

## 前半段：概念與流程

### 一、AI 如何配合自己的能力與需要

剛才的變化需要兩種能力配合：知道內容缺什麼，也知道有哪些工具能協助完成。
...
回到開場所看的三個狀態，再看自己這次的成果：補上了什麼內容、哪一項採用或刪除是自己的判斷，以及哪些搜尋、整理或排版工作交給了 agent。用實際版本指出變化，不預設每個步驟都已省時。

最後以「讓 agent 把你的想法做成成果」收束：這次留下可回查的材料、自己的 draft 與可編輯投影片，下次能接著修改。將一項值得沿用的要求補進 `AGENTS.md`，講師收集下次想處理的工作，練習在課堂完成。

## 講師準備與備講材料

- **課前決定**：實作主題、總時數與配比，準備少量相關公開材料、同題材的三個開場預覽狀態，並測試輸出環境。時間不足就縮減頁數與素材量，保留一條論點到試講的完整體驗。
