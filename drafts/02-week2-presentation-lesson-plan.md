...
講稿先用接近文章的方式，把論點、前後關係與必要說明寫完整。轉成投影片分頁時，再安排每頁口述什麼、畫面呈現什麼，以及停留與換頁的節奏；口述與畫面互相配合，內容也會隨這個節奏重新組織。

請 agent 沿故事線先起草一頁 Markdown 文字，看過寫法再展開其餘頁面，預計三到五頁。

挑一頁補入自己對 repo 做法的理解與實際範例，例如一段內容為什麼適合用比較或流程呈現。直接編輯或請 agent 改寫，引用說明、圖片或 demo 時保留來源；未完成的地方標記待補。
