...
回到開場的研究專案：現在要向同事說明目前的發現，需要做一份簡報。先從已有材料選出要講的內容，補足依據、整理成文章，再請 agent 製作投影片。剛才談的知識協助、工具操作、重複工作與檢查，都可以在這個實例裡用到。
...
| 產出物 | 保存什麼、如何使用 |
|---|---|
| `kb/`：搜尋與探索材料 | 保存文獻、案例、來源、摘要與探索筆記，連到圖片原檔。例如研究方法的文獻、資料說明與分析圖，供後續取用與查核。 |
...
本課自繪的材料判斷方式。把候選圖像放在相關想法旁，記下用途；支持不足就縮小主張或補查。例如只在一批資料上試過的方法，先說清楚測試範圍，不直接推成所有情境都適用。
...
**待修正草稿（課程自擬）：** 這個方法比原本的方法好。

這句話還缺比較依據。回到實際材料，補上用了哪批資料、和哪個方法比較、採用什麼指標，以及觀察到的結果與限制。沒有做過的比較就保留為待驗證問題，不先寫成結論。

現場從實作 draft 挑一句話，對照來源或分析結果，完成這樣的修改並展示前後版本。
...
選實作研究中的一個發現，畫面放可由資料支持的主張與相應圖表，口述交代方法、比較條件及限制。圖表保留座標、圖例與來源，先確認投影時能讀懂。
