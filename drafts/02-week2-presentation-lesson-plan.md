...
> 沒採用、暫緩與被取代的內容見[移出素材與取捨](../docs/week2-deferred-materials.md)；舊版本見[歷史索引](../docs/archive/week2-2026-09-19/README.md)。
...
| 段落 | 時間 | 主線 |
|---|---|---|
| 講課 | 約 30 分 | 能力與需要 → 請教與交辦 → 保存 → 交付責任 → 三圈與製作實例 |
| 示範 | 約 30 分 | 向新進同仁介紹 GDMS，只展開一條論點，示範八步之間的連接 |
| 自由練習 | 約 120 分 | 自選題目，走八步，和一句話生成的版本比較 |

這是第二週原有的三小時安排；第四週另為 09:00–14:00。10–12 頁是練習目標，做多少算多少，不強制交件、兩人互評、GitHub push 或課後作業。研究專案情境保留作口頭補充，不恢復為獨立開場投影片。
...
請教與交辦可以來回切換：討論後有了方向就試做，成果出現新問題再回頭討論。採用什麼、如何修改，仍由自己決定。最省事的交辦是只給一句話、不給材料、不請教，後面的對照組就是這樣做出來的。
...
![簡報內容依序經過確認方向、建立架構與調整風格；每一圈都有人的判斷，未通過時沿虛線回到前面的工作](../assets/week2-diagrams/three-loops-mermaid.svg)

流程圖來源：`assets/week2-diagrams/three-loops.mmd`，與授課投影片使用同一版本。
...
**建立架構：文稿排成逐頁稿，連讀之後調的是順序與取捨。** agent 排出的第一版逐頁稿標題通順但認不出自己的思路，改了七版還不是要的感覺；換成使用者在討論裡說過的原話當底才看得懂，這個做法留成一份 [skill](../skills/ghost-deck-from-own-words.md)。
...
請 agent 找到的每一份材料都實際打開看，整理內容、適用範圍與限制，附來源與查閱日期。區分作者說明、自己實際看見或測試的功能、仍待確認的項目。查找足以支撐目前的論點就先形成草稿，避免一直擴大清單；缺少的材料標記待補，依現場進度安排搜尋時間。材料不只在這一步找：之後任何一步卡住都可以再找一輪，例如排版前先找投影片的原則，就是同一個動作。
...
分頁的原則不必另外找，一頁一句主張、畫面是證據、解釋交給口述，直接寫進對 agent 說的話裡；想看依據可對照課程的[原則筆記](../kb/tools/slide-design-principles-argument-to-slides.md)。
...
調整風格的第二個面向，輸出成品之後才是真正的調風格。排版的標準這門課已經找過一輪，可參考下面這份當時採用的視覺設計規則；也可以請 agent 再找一輪比對，或直接沿用，不理解的條目先拿掉。標準用過確定有效，補進 `AGENTS.md`，下次進到排版直接拿出來，這一步就消失了。

以下是本次製作的風格選擇，不是所有簡報的通則；配色、字級與圖文比例仍依投影環境和內容調整。
...
第一次走這條製作路徑時，先請 agent 做一頁可編輯 PPTX，確認這台機器真的做得出來；路徑確認之後，每次都請 agent 依逐頁稿整份產出。本課示範用程式產出投影片，便於整份重建與檢查一致性；實際耗時仍依工具與內容而異。這次以整份為迭代單位：整份重建，匯出 PDF 整份看。沒有指定版型時，至少交代字體與頁面比例。例如：「照這份逐頁稿做成可編輯的 PowerPoint，16:9，版面照這份規則，做完匯出 PDF 給我看。」規則從第一版就給。
...
有兩件事需要使用者親自確認：親自在 PowerPoint 點選文字、改一個字，確認真的能編輯；自己找時間真的講一遍、計時。這是一個確認，不用兩人一組。講到卡住、講不順的那一頁，內容還不是自己的判斷，回到文稿補材料或刪掉。
...
- 使用獨立示範專案，避免 agent 把這門課的備課紀錄當成 GDMS 材料。先備好 GDMS 公開頁面與說明連結；尚未備妥時用可公開的備用題。
- 開頭的一句話版本另存，與經過判斷的版本區分；產出後實際打開，確認輸出位置、可編輯性與預覽方式。
- 示範只展開一條論點，讓學員看見背景如何成為方向、文稿、逐頁稿與成品。不要把全部時間用於背景搜尋或等待排版。
- 搜尋足夠支持目前論點就往下走，缺口明列。沒有所有人同時停下的檢查點；巡迴協助時依學員卡住的環節處理。
- 找不到方向：先問給誰看、希望對方知道什麼，再挑一段熟悉的材料。產出太空泛：請學員補自己的例子與取捨。版面反覆出錯：整理一組規則後重做。
- 試跑紀錄中的四、五頁案例，題目是簡報形式比較；它們能說明查證與改稿的卡點，不能當成 GDMS 10–12 頁完整實作已驗證的證據。
- 收尾回看三問：方向與洞見是自己的嗎；順序是自己調的嗎；風格照了哪份標準。留下至少一項自己願意再用的要求即可。

## 正式投影片的內容對照

此表保留已授課版本的索引，供改稿時查找，不要求下一版沿用頁數。

| 正式版頁次 | 本稿段落 |
|---|---|
| 1 | 封面與本週目的 |
| 2–4 | 能力互補、請教與交辦、從需要開始 |
| 5–7 | 外部化保存、AGENTS.md、工作紀錄 |
| 8 | 交付物代表自己 |
| 9–21 | 一般簡報流程、三個迴圈、投影片製作實例 |
| 22–23 | GDMS 示範與資料確認 |
| 24–31 | 八步實作 |
| 32–34 | 自選題目、兩版並排與三問回看 |

## 後續修改時的取捨

講課先讓學員知道自己能怎麼用，再以簡報流程連起來；不恢復客服研究、工具品牌巡禮或全部製作歷史。製作歷程只保留能指出「人在這裡做了什麼判斷」的例子。詳細移出內容與候選去向集中於[移出素材](../docs/week2-deferred-materials.md)。

文中的外部參考沿用既有備課來源，本輪未重新驗證產品功能或最新規範；若下一次授課需要講具體功能，再按當時版本查核。
