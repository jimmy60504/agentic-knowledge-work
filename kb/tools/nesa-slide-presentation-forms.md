...
候選四頁：業務簡報的需求／三種形式與優缺點（附真實範例）／套到具體情境如何選／對應工具或製作路徑與本次選擇。三到五頁是介紹比較的成果，不要求三種形式各做一份，也不展開安裝與所有功能。這仍可練習來源整理、建立比較判準、形成建議、完成 draft 與 PPT。尚未替換主 draft 的實作題目。
...
NESA 不作安裝教學或指定產製工具；比較的是三種形式，只交付一份簡報，不各做三套。先前「介紹 NESA repo」與熟悉研究報告的安排由此取代。主稿已列材料可取得、人的取捨、PPTX 可用與節奏的試跑判準；實際試跑尚未執行，工具與耗時不預先宣稱通過。
