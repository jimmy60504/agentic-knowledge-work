...
2. **它的流程跟課程的五步與六層要素是同一張圖。** Story 那一步要求一句核心主張、受眾行動、每頁單一訊息，就是教案的策略層標準；而且它把「先選形式」「補齊受眾與目標」做成阻擋式提問，正是「情境與核心訊息只能人給」的工程化版本。可當「別人也這樣拆」的印證。
3. **大綱規劃的「不強制列點」與十種畫面內容形式**，是「一頁一論點」往下一層的具體清單：這頁的論點該用比較、流程、時間軸還是數據來呈現。可以直接借進第二週的標準或第四週。
...
7. **作者做的事就是課程要學員做的事。** 「把自己的設計觀念變成規範」正是「先會做，再把標準寫進 skill」；他是專家所以寫成七十種版型加契約，學員第一週寫的是三行。他是終點的樣子，課程教的是走到那裡的路。
8. **Skill 與框架的區分就是場景矩陣**：技巧模組對應 skill 檔，場景對應組合 skills 的 workflow，他把後者叫框架。
9. **四個問題可直接當視覺層 slop 症狀，五條設計規範可直接當表達層標準**，比教案現有的「視覺層級：對比、留白、對齊」具體。
...
- 待整理進教案的標準：十種畫面內容形式、Art Direction Brief 的四個欄位。

## 第二週實作安排更新（2026-09-07）

使用者決定以「介紹 NESA-SLIDE repo」作為現場製作簡報的主題，讓內容本身涵蓋簡報的排版方法。此安排已整合第二週主 draft，取代先前只作候選引用、演示主題待選的狀態。

實作從讀 repo、保存來源與觀點開始，人先列綱，再請 agent 整理故事線、起草與協助收斂；排版時把已閱讀與選用的做法應用到這份介紹簡報，對照內容關係、版型與成果。這是介紹與實作主題的決定，具體 skills、工具與安裝方式仍待備課測試；本筆記舊版功能資訊在授課前需依當時 repo 核對。

## 第二週實作題目再討論：三種形式的比較與選擇（2026-09-07，候選）

使用者提出：以介紹目前業務作廣義情境，實作聚焦「agent 如何做簡報」，參考 NESA 的三種形式比較優缺點、可用工具，做三到五頁。這比要求學員自備未完成研究更容易共用材料，且可以把所學直接用於本次成果。

本次重讀 [repo README](https://github.com/slidefirm/NESA-SLIDE)：仍列四種成品，課程可歸成圖片、HTML、PPTX 三大類，HTML 內分純版型與圖片背景。這是輸出形式及製作路徑的比較，不是三種溝通敘事模式；其他工具的當前支援與限制仍需另查，不能只按工具名稱分類。

Agent 建議把核心問題定為「同事要用 agent 做一份業務簡報，該如何選製作方式？」先由人提出會不會持續修改、誰接手、在哪播放、需要什麼視覺等比較標準，再請 agent 讀來源、看範例及補查。由材料形成適用情境與建議，保留取捨，避免只整理工具清單。

候選四頁：業務簡報的需求／三種形式與優缺點（附真實範例）／套到具體情境如何選／對應工具或製作路徑與本次選擇。三到五頁是介紹比較的成果，不要求三種形式各做一份，也不展開安裝與所有功能。這仍可練習來源整理、建立比較標準、形成建議、完成 draft 與 PPT。尚未替換主 draft 的實作題目。

## 最新整合：NESA 作探索起點（2026-09-07）

使用者明確要求：不直接介紹 NESA 怎麼用，以它的分類作起點，整合進實作並準備後續 demo 試跑。主 draft 已更新為「用 agent 做業務簡報，該選哪種方式？」：先定情境與比較條件，讀 NESA 範例，再補查代表工具官方資料，形成建議、文章與三到五頁 PPTX。

NESA 不作安裝教學或指定產製工具；比較的是三種形式，只交付一份簡報，不各做三套。先前「介紹 NESA repo」與熟悉研究報告的安排由此取代。主稿已列材料可取得、人的取捨、PPTX 可用與節奏的試跑標準；實際試跑尚未執行，工具與耗時不預先宣稱通過。
