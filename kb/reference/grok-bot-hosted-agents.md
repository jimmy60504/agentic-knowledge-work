...
- **網路上的示範案例多數是為了示範。** 使用者 2026-09-21 看過分享的 Grok Bot 用例後的感覺：「好像為了玩 bot 硬要去做的一件事情」。判斷一個用例是不是硬做的，可以問一句：拿掉 Agent 之後，這件事還會不會做。原本就在做、做了很久、做得很煩的事，Agent 接手才有差異可比；原本不存在的事，差異無從比較。這與數發部手冊 2.2「避免為導入 AI 而 AI」是同一件事。
- **換的理由來自試用後的差異，不來自跟上時代。** 使用者原話：「不需要說一定要大家就是要把所有東西都換成 agent 才夠跟得上時代，而是真的用了會發現不一樣才換。」這句話是課程對第四到六種介入方式的立場，也是工作坊「未實測的收益視為假設」那一列的依據。
